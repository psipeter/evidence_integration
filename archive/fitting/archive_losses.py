"""
Archived loss functions for the jiang task.
Extracted from fitting/losses.py. See archive_readme.md.
"""
...
def jiang_response_loss(
    params: dict,
    model: pd.DataFrame,
    human: pd.DataFrame,
) -> float:
    """Archived jiang NLL branch from fitting.losses.response_loss()."""
...
def jiang_shape_loss(
    params: dict,
    model: pd.DataFrame,
    human_full: pd.DataFrame,
) -> float:
    """Archived jiang shape branch from fitting.losses.shape_loss()."""
    if "beta" not in params:
...
def _fit_beta(
    ...
    def neg_nll(beta: float) -> float:
        ...
        try:
            return jiang_response_loss(p, model_responses, human)
        except Exception:
            return float("inf")
...
# Archived from fitting/losses.py
# shape_loss and joint_loss were used when loss_type="shape" or "joint".
# loss_type as a concept has been removed; only RMSE fitting remains.
...
def response_loss(
    ...
    """Response-accuracy loss for carrabin and yoo: RMSE."""
...
def joint_loss(
    ...
    if dataset not in ("carrabin", "yoo"):
        raise ValueError(
            f"joint_loss() is only implemented for carrabin and yoo; "
