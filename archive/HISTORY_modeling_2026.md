...
## `presentations/make_figures.py` retired: superseded by `scripts/make_figures.py --mode presentation` (this session)

### What moved

`presentations/make_figures.py`, `git mv`'d verbatim (same filename, no
`archive_` prefix -- a complete, independently-runnable script, not
extracted pieces of a larger file) into `archive/scripts/make_figures.py`:

- `presentations/make_figures.py` -> `archive/scripts/make_figures.py`
  (tracked; moved via `git mv`, history follows).

Its own module docstring, imports, and every `make_*` function are
unchanged -- consistent with this file's own whole-file-move convention
(e.g. the `plot_iti_perturbation.py` entry above), the archived copy is
kept as an accurate historical snapshot rather than edited in place.

### Why

The project generalized `scripts/make_figures.py` (formerly
`make_paper_figures.py`) to support a `--mode {paper,presentation}` CLI
flag, so ONE script now produces both the paper's figures
(`paper/figures/`) and presentation-sized versions of the same figures
(`presentations/figures/`), each with its own width/style -- see
`CLAUDE.md`'s "Figure modes" section and `scripts/make_figures.py`'s own
module docstring. `presentations/make_figures.py` was the OLD, separate,
bespoke script this new capability is meant to replace, and the PI
explicitly said to retire it now.

**Known gap, explicitly accepted by the PI**: `scripts/make_figures.py
--mode presentation` covers the same conceptual role going forward, but
only 2 of the 13 figures `presentations/make_figures.py` used to produce
have actually been ported/regenerated there so far --
`temporal_performance` and `model_performance` (both already regenerated
this session by the new script, confirmed current on disk before this
retirement). The other 11 have NOT been ported and have no
`scripts/make_figures.py` equivalent yet:

- `response_change` (produced two files, `response_change_human.svg`
  advancing to `response_change_full.svg` via an r-stack reveal.js
  fragment build)
- `lambda_human`
- `lambda_sanity_human`
- `lambda_model_correlation`
- `variability_human`
- `variability_models` (not currently referenced by
  `presentations/presentation.qmd` at all -- orphaned output even before
  this retirement, deleted anyway since it's still one of the old
  script's own generated files)
- `sigma_sanity_human`
- `sigma_model_correlation`
- `model_performance_nll`
- `variance_autocorr_human`
- `variance_autocorr_models`

The PI is remaking the actual presentation deck next week and will port
the rest then.

### Generated SVG output deleted, not archived

12 SVG files under `presentations/figures/` -- every output file the old
script's own `savefig`/`FIGURES_DIR / "<name>.svg"` calls create for the
11 not-yet-ported figures above -- were deleted outright (`git rm`), not
moved to `archive/`, per explicit instruction (these are reproducible
build artifacts of the retired script, regenerable once each figure is
ported):

`response_change_full.svg`, `response_change_human.svg`,
`lambda_human.svg`, `lambda_sanity_human.svg`,
`lambda_model_correlation.svg`, `variability_human.svg`,
`variability_models.svg`, `sigma_sanity_human.svg`,
`sigma_model_correlation.svg`, `model_performance_nll.svg`,
`variance_autocorr_human.svg`, `variance_autocorr_models.svg`.

Explicitly NOT touched, per instruction:

- `presentations/images/` -- a wholly separate directory of hand-placed
  visual aids (photos, hand-drawn schematics), not part of this
  retirement at all.
- `presentations/figures/PE_dynamics.svg`, `PE_drives_sigma.svg`,
  `lambda_drives_discounting.svg` -- hand-placed visual aids embedded
  directly in `presentation.qmd`, never generated by any script.
- `presentations/figures/task_model_overview.svg`, `lambda_models.svg`,
  `lambda_sanity_models.svg` -- orphaned/ambiguous: not generated by the
  old script's CURRENT code, not referenced in `presentation.qmd`
  either, but also not confirmed hand-placed visual aids. Left alone
  rather than guessed at.
- `presentations/figures/temporal_performance.{svg,pdf}`,
  `model_performance.{svg,pdf}` -- already regenerated this session by
  the NEW `scripts/make_figures.py --mode presentation`; current/correct
  content, not old-script leftovers, despite two of the four filenames
  overlapping with old-script output names.

### Known side effect, already accepted by the PI

`presentations/presentation.qmd` references the 12 deleted files (among
others) via `![](figures/...)` -- those references are now broken and
will stay broken until each figure is ported to
`scripts/make_figures.py` and the deck is rebuilt, which the PI plans to
do next week. `presentation.qmd` itself was deliberately NOT edited to
work around this.

### Live doc/comment references updated

- `CLAUDE.md`'s "Figure modes" section and "Repository structure" block
  (the `presentations/` entry) both updated to describe the script as
  retired/archived, with the 11-figure porting gap spelled out and a
  pointer to this entry.
- `scripts/make_figures.py`'s own module docstring (which used to
  describe `presentations/make_figures.py` as "still stand[ing] on its
  own" with a "planned retirement") updated to reflect that it has
  already been retired, pointing here for the full narrative.
- Historical narrative mentions of `presentations/make_figures.py` in
  `archive/HISTORY_raw_2026-09.md`, this file's own earlier entries
  above, and inline design-lineage comments inside
  `scripts/make_figures.py` (e.g. "unlike presentations/make_figures.py's
  own TASK_PANELS, which used a 4-model list...") were left as-is,
  per this project's convention of not rewriting historical narrative
  after a later rename/retirement.

### Verification

- Repo-wide grep (excluding `archive/`, `venv/`, `node_modules/`,
  `.git/`) before moving anything confirmed no Python file imports from
  `presentations/make_figures.py` or `presentations.make_figures` --
  it was a standalone, directly-run script with no other consumer.
- `python -m py_compile archive/scripts/make_figures.py` -- clean.
- Re-grepped the whole repo after all edits: the only remaining live
  (non-archive, non-historical) references are the design-lineage
  comments inside `scripts/make_figures.py` noted above (left as
  historical narrative, not stale current-state claims) and
  `presentation.qmd`'s now-broken `![]()` references (left broken, per
  instruction).

### No data files moved

Nothing in `data/` or `data/runs/` changed. This retirement only touched
`presentations/make_figures.py` itself and its own SVG output under
`presentations/figures/`.

### How to restore

`git mv archive/scripts/make_figures.py presentations/make_figures.py`.
Re-add the CLAUDE.md/`scripts/make_figures.py` docstring text describing
it as active (this entry's own diff, reverted). The 12 deleted SVGs are
NOT recoverable from `archive/` (they were deleted, not archived) --
re-run `venv/bin/python presentations/make_figures.py <name>` for
whichever ones are needed once the script is restored.
