...
# ---------------------------------------------------------------------------
# Pool path: the REAL, promoted 200-per-task hybrid pool -- each pool
# member is one genuinely independent participant's assigned sequence set
# (see timeline-builder.js's poolIndexForParticipant), unlike the iid path
# above (which regenerates fresh i.i.d. draws on every run) and unlike the
# quota path (a single shared file). model_id is set to the REAL pool
# member index (p{NNNN}, matching the file's own {task}_{NNNN}_sequences.*
# naming) rather than a simulation-order index, so a given row here is
# always traceable back to the exact file it came from.
# ---------------------------------------------------------------------------
def load_pool_sequences(tasks, pool_dir, agent_name, agent_fn, skip_binary_transform=False,
                        n_pool=None):
    """Load every (or the first n_pool, if given) {task}_{NNNN}_sequences.pkl
    under pool_dir -- the REAL pool generated by generate_sequences_pool.py
    (currently wrapping generate_sequences_hybrid.py's per-task methods) --
    and build the same long-format rows simulate_participants builds for
    iid, so all three sequence_types are directly comparable through this
    script's SAME downstream fitting/plotting code. Unlike --sequence_type
    iid (which regenerates FRESH i.i.d. draws every run, never touching
    disk), this reads the ACTUAL files that will be/are served to real
    participants -- the right tool for "what does our real deployed pool
    actually look like", not a simulation of what a pool COULD look like.
    """
    pool_dir = Path(pool_dir)
    all_rows = []
    for task in tasks:
        pkls = sorted(pool_dir.glob(f"{task}_*_sequences.pkl"))
        assert pkls, f"no pool files found matching {task}_*_sequences.pkl in {pool_dir}"
        if n_pool is not None:
            pkls = pkls[:n_pool]
        for pkl in pkls:
            # Filename is {task}_{NNNN}_sequences.pkl -- NNNN is the real
            # pool index, extracted here (not just enumerated) so model_id
            # always matches the actual file, even if n_pool slices the list.
            idx = pkl.stem.split("_")[1]
            df = pd.read_pickle(pkl)
            all_rows.extend(_rows_for_sequences(df, task, f"p{idx}", agent_name, agent_fn,
                                                skip_binary_transform))
    return pd.DataFrame(all_rows)
...
def make_figure(raw_df, fits_df, tasks, out_path, n_participants, agent_label="Mean",
    ...
    elif sequence_type == "pool":
        fig.suptitle(
            f"REAL hybrid pool ({n_participants} actual pool members loaded from "
            f"disk, not simulated)  |  agent: {agent_label}",
            fontsize=11, fontweight="bold")
    else:
        fig.suptitle(
...
def parse_args():
    ...
    p.add_argument("--sequence_type", choices=["iid", "quota", "pool"], default="iid",
                  help="'iid' (default): --n_participants independent i.i.d. draws, "
                       ...
                       "'pool': the REAL per-participant hybrid pool (task/"
                       "sequences_pool/, generated by generate_sequences_pool.py) -- "
                       "reads the actual files real participants get assigned, not a "
                       "fresh simulation; --n_participants caps how many pool members "
                       "to load (default: all of them).")
    ...
    p.add_argument("--pool_dir", default="task/sequences_pool",
                  help="Only used when --sequence_type pool")
...
def main():
    ...
    elif args.sequence_type == "pool":
        print(f"Loading real pool sequences from {args.pool_dir} "
              f"x {len(tasks)} task(s) | agent={agent_name}...")
        raw_df = load_pool_sequences(
            tasks, args.pool_dir, agent_name, agent_fn,
            skip_binary_transform=args.skip_binary_transform,
            n_pool=None if args.n_participants == 50 else args.n_participants)
        # n_participants (for output naming/titles below) reflects how many
        # pool members actually got loaded, not the CLI default -- 50 is
        # this script's default for the IID path, not a meaningful cap here
        # unless the user actually passed it.
        args.n_participants = raw_df["model_id"].nunique()
    else:
