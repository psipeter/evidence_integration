...
============================
Run NEF on test sequences for one (alpha_0, lambda_) param set.
Saves to a separate per-param pkl so parallel runs don't conflict.
Collect results with scripts/collect_nef_sequences.py afterwards.

Usage (one per terminal):
    venv/bin/python scripts/run_nef_sequences.py --alpha_0 0.3 --lambda_ 0.2
    venv/bin/python scripts/run_nef_sequences.py --alpha_0 0.3 --lambda_ 0.5
    venv/bin/python scripts/run_nef_sequences.py --alpha_0 0.3 --lambda_ 0.8
    venv/bin/python scripts/run_nef_sequences.py --alpha_0 0.6 --lambda_ 0.2
    ...etc

Output: data/runs/test_sequences/nef_runs/nef_a{alpha_0}_l{lambda_}.pkl
"""
...
def main():
    ...
    parser.add_argument('--tasks',   nargs='+',  default=['task_continuous', 'task_binary'])
    args = parser.parse_args()

    alpha_0   = args.alpha_0
    lambda_   = args.lambda_
    tag       = f'a{alpha_0:.3f}_l{lambda_:.3f}'.replace('.','p')
    params_str = f'alpha_0={alpha_0:.3f} lambda_={lambda_:.3f}'

    out_folder = resolve_run_folder('test_sequences') / 'nef_runs'
    out_folder.mkdir(parents=True, exist_ok=True)
    out_path = out_folder / f'nef_{tag}.pkl'

    if out_path.exists():
        print(f"Already exists: {out_path} — skipping.")
        return

    print(f"Running NEF: alpha_0={alpha_0}  lambda_={lambda_}")
