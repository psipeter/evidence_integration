...
for the bottom panels of figure_carrabin. It must be run BEFORE
``scripts/figure_carrabin.py`` to generate the required probe pickle files.
...
One SLURM job per pid.
  ...
  Collect / plot (after all jobs complete):
      python scripts/extras_carrabin.py \\
          --experiment probe_pids \\
          --run_folder refit \\
          --out_folder refit
...
These files are read by scripts/figure_carrabin.py to generate the bottom
