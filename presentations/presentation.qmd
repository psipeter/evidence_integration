---
title: "How People Learn"
subtitle: "from Streams of Information"
author: "Peter Duggins"
format: 
  revealjs:
    theme: [default, custom.scss]
    logo: figures/logo.svg
    incremental: true 
    highlight-style: github
    slide-number: true
    html-math-method: katex
    include-after-body: include-scripts.html
    # embed-resources: true
    revealjs-plugins:
      - pointer
bibliography: "references.bib"

---

## Cognitive Neuroscience

:::: {.columns}

::: {.column width="50%"}
::: {.image-card}
![](figures/cognitive_science_loop.svg){style="height: 480px; width: auto;"}
:::
:::

::: {.column width="50%"}
::: {.image-card}
![](figures/brain_model_behavior.svg){style="height: 480px; width: auto;"}
:::
:::

::::

## Evidence Integration

::: {.image-card}
![](figures/evidence_integration_examples.jpg){width="100%"}
:::

## Behavioral Experiments

::: {.image-card}
![](figures/tasks_overview_presentation.svg){width="100%"}
:::

## People Learn as they Gather Data

::: {.image-card}
![](figures/temporal_performance.svg){width="100%"}
:::

## Cognitive Heuristics

::: {.image-card}
![](figures/complexity_1.svg){#theories-img width="100%"}
:::

[]{.fragment fragment-index="1" data-swap-target="theories-img" data-swap-src="figures/complexity_2.svg" data-swap-prev="figures/complexity_1.svg"}
[]{.fragment fragment-index="2" data-swap-target="theories-img" data-swap-src="figures/complexity_3.svg" data-swap-prev="figures/complexity_2.svg"}
[]{.fragment fragment-index="3" data-swap-target="theories-img" data-swap-src="figures/complexity_4.svg" data-swap-prev="figures/complexity_3.svg"}
[]{.fragment fragment-index="4" data-swap-target="theories-img" data-swap-src="figures/complexity_5.svg" data-swap-prev="figures/complexity_4.svg"}


## Theory and Model

::: {.image-card}
![](figures/models_overview_presentation_1.svg){#models-overview-img width="100%"}
:::

[]{.fragment fragment-index="1" data-swap-target="models-overview-img" data-swap-src="figures/models_overview_presentation_2.svg" data-swap-prev="figures/models_overview_presentation_1.svg"}
[]{.fragment fragment-index="2" data-swap-target="models-overview-img" data-swap-src="figures/models_overview_presentation_3.svg" data-swap-prev="figures/models_overview_presentation_2.svg"}

## Model Captures Behavior

::: {.image-card}
![](figures/model_performance.svg){width="100%"}
:::


## Smaller Updates Over Time ($\lambda$)

::: {.image-card}
![](figures/lambda_response_change.svg){width="100%"}
:::

## Individual Differences in $\lambda$

::: {.image-card}
![](figures/lambda_distributions_edited.svg){width="100%"}
:::

## People have Noisy Responses ($\sigma$)

::: {.image-card}
![](figures/sigma_distributions_edited.svg){width="100%"}
:::

## Response Noise Autocorrelation

::: {.image-card}
![](figures/sigma_growth_autocorr.svg){width="100%"}
:::

## Prediction Error & Neural Activity

::: {.image-card}
![](figures/PE_dynamics.svg){width="100%"}
:::

## PE variance drives $\sigma$

::: {.image-card}
![](figures/PE_drives_sigma.svg){width="100%"}
:::

## $\lambda$ drives discounting

::: {.image-card}
![](figures/lambda_drives_discounting.svg){width="100%"}
:::

## Conclusions

:::: {.columns style="display: flex; gap: 2em;"}

::: {.column width="50%"}
#### Evidence Integration

::: {.nonincremental}
* Error-driven learning
* Four cognitive tasks
* Individual differences
:::
:::

::: {.column width="50%"}
#### Spiking Neural Network

::: {.nonincremental}
* Captures human data
* Explains neural mechanisms
* Makes novel predictions
:::
:::

::::

::: {.image-card style="margin: 1em auto 0 auto;"}
![](figures/snn.svg){style="height: 200px; width: auto; display: block; margin: 0 auto;"}
:::


## Acknowledgments {.smaller}

::: {style="display: flex; justify-content: space-evenly; width: 100%; min-width: 100%; box-sizing: border-box;"}
![Alireza Soltani](figures/alireza.jpg){style="height: 110px; width: 110px; object-fit: cover; border-radius: 8px;"}
![Peter Duggins](figures/peter.jpg){style="height: 110px; width: 110px; object-fit: cover; border-radius: 8px;"}
![Dartmouth logo](figures/logo.png){style="height: 110px; width: 110px; object-fit: cover; border-radius: 8px;"}
![NSF logo](figures/nsf_logo.png){style="height: 110px; width: 110px; object-fit: cover; border-radius: 8px;"}
:::


Neural Engineering Framework

::: {style="font-size: 0.6em;"}
Eliasmith, Chris, and Charles H Anderson. 2003. Neural Engineering: Computation, Representation, and Dynamics in Neurobiological Systems. Cambridge, MA: MIT press.
:::

Balls Task / Data

::: {style="font-size: 0.6em;"}
Prat-Carrabin, Arthur, and Michael Woodford. 2024. “Imprecise Probabilistic Inference from Sequential Data.” Psychological Review 131.
:::

Snacks Task / Data

::: {style="font-size: 0.6em;"}
Yoo, Minhee, Giwon Bahg, Brandon Turner, and Ian Krajbich. 2025. “People Display Consistent Recency and Primacy Effects in Behavior and Neural Activity Across Perceptual and Value-Based Judgments.” Cognitive, Affective, & Behavioral Neuroscience 25.
:::

## Individual Differences in $\lambda$

::: {.image-card}
![](figures/lambda_human.svg){width="100%"}
:::

## $\lambda$ Consistent Over Trials/Tasks

::: {.image-card}
![](figures/lambda_sanity_human.svg){width="100%"}
:::

## Models Capture $\lambda$ Differences

::: {.image-card}
![](figures/lambda_model_correlation.svg){width="100%"}
:::

## $\sigma$ Consistent Over Trials/Tasks

::: {.image-card}
![](figures/sigma_sanity_human.svg){width="100%"}
:::

## Models Capture $\sigma$ Differences

::: {.image-card}
![](figures/sigma_model_correlation.svg){width="100%"}
:::