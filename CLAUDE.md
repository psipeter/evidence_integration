...
- **Figures save PDF only** — never PNG, never upload images to chat
  ...
  `scripts/make_figures.py` saves **SVG only**, in BOTH `--mode paper` and
  `--mode presentation` (see "Figure modes" below) — a standing house
  convention for this script specifically, not a license to add PNG/SVG
  output elsewhere. For `--mode paper` specifically: every figure's own
  bold panel labels (A/B/C...) are now added BY HAND in an SVG editor,
  not by this script (repeated automated-placement attempts never
  reliably got them positioned above and to the left of all panel
  content) — so main.tex's own `{stem}.pdf` is a MANUALLY produced file
  (the person's own hand-labeled export of this script's freshly
  generated `{stem}.svg`), regenerated by the person whenever the
  underlying `.svg` changes, not something Claude ever writes or commits.
...
| Format | SVG only | SVG only |

`--mode paper`'s own `{stem}.svg` carries NO panel labels (A/B/C...) —
the person adds those by hand in an SVG editor and exports the result as
`paper/figures/{stem}.pdf`, which is what `main.tex` actually embeds.
Regenerating a figure's `.svg` after a data/code change means that PDF
is stale until the person re-labels and re-exports it — `make_figures.py`
itself never writes a `{stem}.pdf`. See `utils/plot_style.py`'s
(unused-by-this-script) `label_panels` for the retired auto-placement
attempt, kept only because `scripts/inspect_participant_temporal.py`
still calls it for its own (non-paper) diagnostic figures.
...
own SVG-only exception, both modes).
