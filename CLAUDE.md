...
- Continuous task: Normal(mean, std) stimulus; slider response [0–100]; 40 trials × 15 obs
...
Sequence design: 10 unique × 4 repeats, 15 obs, prefix_length=4, std_fixed=20.
Seeds: continuous=934, binary=1229. All trials use ITI_MS=1000ms (ITI manipulation removed).
...
- jatos.js must be in HTML <head> (already in index-{task}.html)
...
- Free task ordering — natural counterbalancing via Prolific dashboard randomisation
- Both set to auto-approve so second task appears on dashboard immediately after first
- End screen: 'one half of a two-part study — look for the other on your dashboard'
- Payment: ≥$12/hr recommended; set estimated time conservatively (e.g. 35 min for 30 min task)
...
### Generate sequences
    ...
    venv/bin/python task/generate_sequences.py --task continuous --seed 934
    venv/bin/python task/generate_sequences.py --task binary --seed 1229
