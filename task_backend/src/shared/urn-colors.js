/**
 * urn-colors.js
 * Shared probability-bar SVG builder for the colors tutorial.
 *
 * Used by:
 *   plugin-tutorial-intro-colors.js       (progressive reveal via group opacity)
 *   plugin-tutorial-observation-colors.js (fully revealed from the start)
 *
 ...
 * (colors-draw-animation.js) conveys the generative process instead.
 *
 * SVG groups / elements:
 *   #tut-urn-bar       — blue/red bar (no label). The two rects inside now
 *                        have their OWN ids (#tut-urn-bar-blue/-red, chat
 *                        history) -- colors-draw-animation.js's resolve
 ...
 * "last" (dimmed) mode (see style.css's input[type="range"].colors-
 ...
 * second one. See colors-draw-animation.js's own docstring for the full
 * mechanism.
 *
 ...
 * (stretched/squashed text), mirrors distribution-numbers.js's
 ...
 * colors-draw-animation.js's svgRoot) still resolves correctly through
 ...
 * colors.js's onBox1()) now ALSO needs to flip the label span's opacity
...
// -- used by colors-draw-animation.js's resolve step to dim whichever bar
...
// .colors-slider.slider-last track rule) -- reusing that existing
...
export const buildUrnSVG = (p, revealed = false) => {
  ...
  // note for why, mirrors distribution-numbers.js's identical helper.
