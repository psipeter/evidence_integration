/**
 * numbers-draw-animation.js
 * Bubbling generative animation for the numbers tutorial — mirrors
 * colors-draw-animation.js's overall structure, but bubbles drift DOWNWARD
 ...
 * numbers draw instead of two discrete regions.
 *
 ...
 *      at the exact observed value by distribution-numbers.js — no
 *      randomness needed here, unlike the colors version, since the actual
 *      value is a specific number, not one of two regions) fades in, and the
 *      centre observation number fades in simultaneously.
 */

import { LAYOUT } from './distribution-numbers.js';
 ...
 *   (see tutorial-tracker.js/plugin-tutorial-*-numbers.js chat history):
 ...
 * @returns {() => void} cancel
 */
export function startNumbersDrawAnimation({
  ...
  // opacity 0 by default (see distribution-numbers.js)
