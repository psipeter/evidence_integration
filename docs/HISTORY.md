# docs/HISTORY.md — task/ (JATOS) build history

**This file exists so CLAUDE.md can stay focused on the CURRENT
architecture (task_backend) without losing the full design rationale,
rejected alternatives, and real-bug narratives that got the project here.**
Nothing in this file describes anything still active -- `task/` (the
JATOS/MindProbe pipeline documented below) is retired; see CLAUDE.md's
"Online task: task_backend" section for what's actually live today, and
`task_backend/TODO.md` for that system's own build log and current status.
...
- You're doing archaeology on a specific past bug or design debate (the
  ...
  etc.) and want the full account rather than a summary.
