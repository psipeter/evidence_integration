...
def fit(
    ...
    for Optuna to search -- a "does adding noise alone explain the NLL
    improvement, or do the OTHER parameters need to move too" check. Base
    model name comes from `base_model_of(model_type)` (strips "_resp_noise").
    The RMSE params file read is `{base_model}_{stem}_{pid}_params.pkl`
    under `override_from_folder` -- NOT `{model_type}_...`, since the
    override source is the UNWRAPPED model's own RMSE fit, never an NLL fit
    of anything.
